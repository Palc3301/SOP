...
export class GerenciadorMemoria {
  ...
  constructor() {
    const quantidadePaginas = GerenciadorMemoria.TAMANHO_MEMORIA / GerenciadorMemoria.TAMANHO_PAGINA

    this.memoriaFisica = new Array(quantidadePaginas)
    for (let frame = 0; frame < this.memoriaFisica.length; frame++) {
      this.memoriaFisica[frame] = new Array<undefined>(GerenciadorMemoria.TAMANHO_PAGINA)
    }
  ...
  public ler(processo: Processo) {
    ...
    for (let i = 0; i < processo.getSubProcesso.length; i++) {
      ...
      const enderecoSubProcesso = this.memoriaLogica.get(subProcessoIdSelecionado)

      if (
        enderecoSubProcesso &&
        this.memoriaFisica[enderecoSubProcesso.quadro][enderecoSubProcesso.indice]
      ) {
        subProcessos.push(
  ...
  public verificarEscrita(processo: Processo) {
    ...
    if (framesVazios.length < processo.getTamanho / GerenciadorMemoria.TAMANHO_PAGINA) {
      return false
    } else {
      return true
    }
  ...
  private alocarProcessoComPaginacao(processo: Processo) {
    ...
    for (let i = 0; i < framesVazios.length; i++) {
      ...
      while (indicePagina < pagina.length && contadorTamanho < processo.getTamanho) {
  ...
  private imprimirMemoria() {
    ...
    for (let pagina = 0; pagina < this.memoriaFisica.length; pagina++) {
      ...
      console.log(pagina+" "+elemento.map(e=>{
        if (e) {
          return e
        } else{
          return " - "
